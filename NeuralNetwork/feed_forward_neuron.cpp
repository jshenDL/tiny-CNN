...
void Feed_Forward_Neuron::run(int mode) {
    ...
    for (auto lk : in_links) {
        total += lk.weighted_in_value(
                0);   //each link to this node has a weight and it needs to multiply the value from the in node
    }
    values[mode] = transfer_function(total);   //this is the activation step, weights is updated for a given mode
}

std::string Feed_Forward_Neuron::get_name() {
